...
function RedisStore() {
  ...
  self.save$ = function(args, cb){
    ...
    // eyes.inspect(objectMap, "save objectmap:");
...
var makeent = function(ent, row) {
  return ent.make$(JSON.parse(row));
};
*/
...
var makeentp = function(ent) {
    ...
    fields.forEach(function(field){
      ...
      //eyes.inspect([ent[field], typeof(ent[field]) ], "field:");
    });
...
var makeent = function(ent, row, objMap) {
  ...
  var objectTypeDate = 'd';
    ...
    row = JSON.parse(row);
    //eyes.inspect(row, "makeent row:");
    //var fields = ent.fields$();
    var fields = _.keys(row);
    //eyes.inspect([fields, row, objMap], "Fields, row, map");
    ...
    if( !_.isUndefined(ent) && !_.isUndefined(row) ) {
      ...
      fields.forEach(function(field){
        if( !_.isUndefined(row[field]) && !_.isUndefined(objMap.map[field])) {
          ...
          //eyes.inspect([ent[field],row[field]], "Field is:");
        }
      });
    }

    //eyes.inspect([ent, row, ent.fields$()], "ent, row:");
...
var determineObjectMap = function(ent){
  ...
  if(!_.isUndefined(globalObjectMap[objectName])  &&
    ...
    //eyes.inspect([objectMap, _.size(globalObjectMap[objectName].map), _.size(objectMap.map)], "Ignored as:");

    return globalObjectMap[objectName];
  } else {
    //eyes.inspect([objectMap], "Saved as:");
